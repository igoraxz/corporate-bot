...
from config import PHONE_ORG_SURNAME, PHONE_DEFAULT_GENDER, ORG_NAME, PHONE_VAPI_MODEL

log = logging.getLogger(__name__)

# Active call state: call_id → {objective, to_number, source, started_at, drop_event, monitor_task, ...}
...
# Base persona prompt for the AI phone agent — {org_surname} is filled at module load
_SURNAME = PHONE_ORG_SURNAME or ORG_NAME
AGENT_PERSONA = f"""You are {{agent_name}}, a personal assistant to {_SURNAME}.
You are making a phone call on their behalf.

IDENTITY:
- Your name is {{agent_name}}. If asked, you work as a personal assistant for the organization.
- If asked "are you an AI?" or "are you a robot?", say: "No, I'm {{agent_name}}, {_SURNAME}'s personal assistant."
...
- You can wait on hold for up to 45 minutes — be patient, don't hang up.
...
- If you're asked something you don't know: say "I'd need to check that with the team and get back to you" — NEVER make things up.
...
- NEVER share: office addresses, personal email addresses, personal phone numbers, financial details, salary info, employee IDs, proprietary data, internal systems details.
...
- If asked for information you're not authorized to share, say: "I don't have that to hand, I'd need to check with the team."
...
async def _send_call_update(call_data: dict, msg: str):
    ...
    try:
        if call_data.get("source") == "teams":
            from integrations.teams import get_client
            client = get_client()
            if client:
                await client.send_message(call_data.get("chat_id", ""), msg, text_format="plain")
        else:
...
async def make_call(
    ...
    Args:
        ...
        source: Originating platform (telegram/teams)
    """
    ...
    if authorized_info:
        ...
        system_prompt += (
            f"\nAUTHORIZED INFO: The organization name is {_SURNAME}. "
            "Beyond that, say \"I'd need to check with the team\" for any personal details.\n"
        )
    ...
    try:
        ...
        if resp.status_code not in (200, 201):
            log.error(f"Vapi API error: status={resp.status_code}")
            return {"error": f"Vapi API error: HTTP {resp.status_code}"}
...
def handle_call_event(event: dict) -> dict | None:
...
def get_call_info(call_id: str) -> dict | None:
