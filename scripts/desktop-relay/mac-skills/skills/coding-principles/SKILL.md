...
```
Security / auth / concurrency / shared state?
...
`docker-compose`, `.env`, credentials, auth/permission logic, deploy scripts, MCP tool security,
shared state / concurrency / module-level globals, access control policy definitions, credential
storage, sandbox configurations.
...
After completing EACH pipeline stage, record it on the PM ticket's TCD:
```
mark_gate(tcd, "stage_9", status="passed", commit="abc1234", notes="quality-reviewer agent")
pm_update_ticket(ticket_id=N, tcd=json.dumps(tcd))
```

The TCD's `pipeline_gates` dict tracks every gate: status (passed/failed/skipped/invalidated),
commit hash, and timestamp. This ensures NO stage is silently skipped — even in long
multi-task sessions where context drifts.
...
- 🔴 SIGNIFICANT: ALL stages (T0, SM, stage_0 through stage_12)
